...
Font* Font::create(const char* family, Style style, unsigned int size, Glyph* glyphs, int glyphCount, Texture* texture)
{
	...
	for( size_t i = 0; i < glyphCount; i++ )
		if( maxCode < glyphs[i].code )	maxCode = glyphs[i].code;

	font->_glyphs.resize(maxCode);
	for( size_t i = 0; i < glyphCount; i++ )
	{
		unsigned int shiftCode = glyphs[i].code - 32; // HACK for ASCII;
		memcpy( &font->_glyphs[shiftCode] , &glyphs[i], sizeof(Glyph) );
	}
    //font->_glyphCount = glyphCount;
	font->_glyphCount = maxCode;
...
// draw Unicode 
void Font::drawText(const wchar_t* text, int x, int y, const Vector4& color, unsigned int size, bool rightToLeft)
{
    if (size == 0)
        size = _size;
    GP_ASSERT(_size);
...
void Font::drawText(const wchar_t* text, const Rectangle& area, const Vector4& color, unsigned int size , 
                Justify justify , bool wrap, bool rightToLeft, const Rectangle* clip)
{
    GP_ASSERT(text);
...
void Font::measureText(const wchar_t* text, unsigned int size, unsigned int* width, unsigned int* height)
{
    GP_ASSERT(_size);
    GP_ASSERT(text);
    GP_ASSERT(width);
...
void Font::measureText(const wchar_t* text, const Rectangle& clip, unsigned int size, Rectangle* out, Justify justify, bool wrap, bool ignoreClip)
{
    GP_ASSERT(_size);
    GP_ASSERT(text);
    GP_ASSERT(out);
...
void Font::getMeasurementInfo(const wchar_t* text, const Rectangle& area, unsigned int size, Justify justify, bool wrap, bool rightToLeft,
        std::vector<int>* xPositions, int* yPosition, std::vector<unsigned int>* lineLengths)
{
    GP_ASSERT(_size);
    GP_ASSERT(text);
...
int Font::getIndexAtLocation(const wchar_t* text, const Rectangle& area, unsigned int size, const Vector2& inLocation, Vector2* outLocation,
                                      Justify justify, bool wrap, bool rightToLeft)
{
    return getIndexOrLocation(text, area, size, inLocation, outLocation, -1, justify, wrap, rightToLeft);
}
...
void Font::getLocationAtIndex(const wchar_t* text, const Rectangle& clip, unsigned int size, Vector2* outLocation, const unsigned int destIndex,
                              Justify justify, bool wrap, bool rightToLeft)
{
    getIndexOrLocation(text, clip, size, *outLocation, outLocation, (const int)destIndex, justify, wrap, rightToLeft);
}
...
int Font::getIndexOrLocation(const wchar_t* text, const Rectangle& area, unsigned int size, const Vector2& inLocation, Vector2* outLocation,
                                      const int destIndex, Justify justify, bool wrap, bool rightToLeft)
{
    GP_ASSERT(_size);
    GP_ASSERT(text);
...
unsigned int Font::getTokenWidth(const wchar_t* token, unsigned int length, unsigned int size, float scale)
{
    GP_ASSERT(token);
    GP_ASSERT(_glyphs.size());
...
unsigned int Font::getReversedTokenLength(const wchar_t* token, const wchar_t* bufStart)
{
    GP_ASSERT(token);
    GP_ASSERT(bufStart);
...
int Font::handleDelimiters(const wchar_t** token, const unsigned int size, const int iteration, const int areaX, int* xPos, int* yPos, unsigned int* lineLength,
                          std::vector<int>::const_iterator* xPositionsIt, std::vector<int>::const_iterator xPositionsEnd, unsigned int* charIndex,
                          const Vector2* stopAtPosition, const int currentIndex, const int destIndex)
{
    GP_ASSERT(token);
